...
The Library `struct` should have a field to store all books (use a `map` with `book ID` as the key) and a field to store members (use a `map` with `member ID` as the key).
- Methods:
...
## Console Interaction:
Create a simple console interface to interact with the library management system.
